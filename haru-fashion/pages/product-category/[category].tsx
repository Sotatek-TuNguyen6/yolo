...
const ProductCategory: React.FC<Props> = ({
  ...
  const firstIndex = page === 1 ? page : (page - 1) * 10 + 1;
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  if (paramCategory !== "new-arrivals") {
    try {
      const numberOfProductsResponse = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/products/count/get?category=${paramCategory}`
      );
      // console.log(numberOfProductsResponse.data)
      numberOfProducts = +numberOfProductsResponse.data.data;
    } catch (error) {
      return {
        notFound: true,
      };
  ...
  const fetchedProducts = res.data.data.map((product: apiProductsType) => ({
    ...product,
    img1: product.images[0]?.url[0] || "",
    img2: product.images[1]?.url[0] || "",
  }));
